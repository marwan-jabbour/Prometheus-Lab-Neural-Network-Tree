...
class DirectionClassifierNNTest {
    static double[][]testSet;
    @BeforeAll
    static void setUp(){
        DirectionClassifierNN aNeuralNetwork= new DirectionClassifierNN();
    ...
    void predictOutputTest_1() {
        int index=0;
        double[]aInput=new double[4];
        for (int i=0;i<4;i++){
            aInput[i]=testSet[index][i];
        }
        double aExpectedOutput=testSet[index][4];
